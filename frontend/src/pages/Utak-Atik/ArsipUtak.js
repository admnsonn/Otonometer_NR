...
const Utakmain = () => {
  ...
  function ItemDropdown({anaknyafilter}){
    const [children, setChildrend] = useState([]);
    const [viewChildren, setViewChildren] = useState(false);
    useEffect(() => {
        fetch(`https://api.otonometer.neracaruang.com/api/filter-child?satuan_id=1&lang=en&parent_id=${anaknyafilter.id}`)
        .then((response) => response.json())
        .then((result) => setChildrend(result.data))
    }, []);

    function showChildren(elm, target, id){
    ...
    <div key={anaknyafilter?.id} className="flex items-center">
        <input
            ...
            onChange={e => showChildren(e.target, `children_${anaknyafilter?.id}`, anaknyafilter?.id)}
    ...
    {viewChildren && (
        ...
        {children.map((child) => (
            <div className="ms-2" >
                <ItemDropdown anaknyafilter={child} />
            </div>
        ))}
        </div>
    )}
            ...
            <div className="bg-third p-[10px] rounded-[10px] text-white">
              {Array.isArray(childFilter) && childFilter.length > 0 ? (
                ...
                childFilter.map((anaknyafilter) => (
                    ...
                    <ItemDropdown anaknyafilter={anaknyafilter} />
                    </div>
                ))
              ) : (
