...
const Daftar = (province_id) => {
  ...
  const fetchCities = async (provinceId) => {
    try {
      const response = await axios.get(
        `https://api.otonometer.neracaruang.com/api/all_cities?province_id=${provinceId}`
      );
  ...
  const fetchProvinces = async () => {
    try {
      const response = await axios.get(
        "https://api.otonometer.neracaruang.com/api/provinces"
      );
      setFormData((prevState) => ({
        ...prevState,
  ...
  const handleChangeCities = (e) => {
    ...
    setFormData((prevState) => ({
      ...
      city_value: selectedCity ? selectedCity.city_value : "",
    }));
  };
    ...
    try {
      if (step === 1) {
        const response = await axios.post(
          "https://api.otonometer.neracaruang.com/api/register",
          {
  ...
  const sendOTP = async () => {
    try {
      const response = await axios.post(
        "https://api.otonometer.neracaruang.com/api/send-otp",
        { email }
      );
  ...
  const checkOTP = async () => {
    try {
      const response = await axios.post(
        "https://api.otonometer.neracaruang.com/api/check-otp",
        { email: email, otp: document.getElementById("otp").value },
    ...
    form = (
      ...
      >
        ...
        <div className="flex mt-4 w-full py-2 px-3 justify-center items-center ">
          ...
          />
          <span className="text-24445A text-sm font-regular text-secondary">
            atau
          </span>
          <hr
        ...
        <div className="flex flex-col">
          ...
          <div className="mb-[10px]">
            ...
            <div className="infield leading-tight focus:outline-none focus:shadow-outline font-regular">
              ...
              <FontAwesomeIcon
                icon={faCalendar}
                className="text-gray-400 ml-[170px]"
              />
            </div>
          </div>
